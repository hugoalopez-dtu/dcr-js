...
var computeStepReward = (result, pendingBefore, executedInEpisode2, eventCost, eventDuration, costWeight = 0, durationWeight = 0) => {
  if (result.reward === -10) {
  ...
  const costPenalty = costWeight > 0 && eventCost !== void 0 ? costWeight * eventCost : 0;
  const durationPenalty = durationWeight > 0 && eventDuration !== void 0 ? durationWeight * eventDuration : 0;
...
app.post("/action", (req, res) => {
  try {
    ...
    const { stepReward, baseMapped, noveltyDelta, progressDelta, costPenalty, durationPenalty } = computeStepReward(rewardInput, pendingBefore, executedInEpisode, eventCost, eventDuration, COST_WEIGHT, DURATION_WEIGHT);
